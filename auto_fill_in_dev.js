...
function setTimer() {
    rawInput('填报时间（小时与分钟均用两位数字表示）：', storage.get('runAt').hour + ':' + storage.get('runAt').minute, input => {
        if (new RegExp(/^(?:(?:[0-2][0-3])|(?:[0-1][0-9])):[0-5][0-9]$/).test(input)) {
            ...
            else if (input === new Date().toTimeString().substring(0, 5)) {
                if (input.substring(3, 5) === '59') {
                    if (input === '23:59') {
                        runAt.setHours(0);
                        runAt.setMinutes(1);
                    }
                    else {
                        runAt.setHours(parseInt(input.split(':')[0]) + 1);
                        runAt.setMinutes(0);
                    }
                }
                else
...
if (typeof storage.get('runAt') === 'undefined')
    ...
    let h = parseInt(storage.get('runAt').hour);
    let m = parseInt(storage.get('runAt').minute);
...
let w = floaty.rawWindow(
    <vertical gravity="center" alpha="0.5">
...
function fillIn() {
    ...
    let fillInBtn = className('android.view.View').text('黑龙江科技大学2021年暑期学生健康数据填报')
        .findOne(5000);
    if (fillInBtn) {
        fillInBtn.click();
        sleep(2000);
    ...
    let todayBtn = className('android.view.View').text('今天')
        .findOne(5000);
    if (todayBtn) {
        todayBtn.click();
        sleep(2000);
    ...
    let submitBtn = text('提交').findOne(5000);
    if (submitBtn) {
        submitBtn.click();
        toastInfo('填报完成。');
    }
